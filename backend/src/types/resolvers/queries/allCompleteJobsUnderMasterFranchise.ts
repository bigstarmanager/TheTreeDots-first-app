...
export const allCompleteJobsUnderMasterFranchise = queryField((t) => {
  t.field('allCompleteJobsUnderMasterFranchise', {
    type: 'TlJobList',
    args: {
      userId: intArg(),
    ...
    resolve: async (
      ...
      const where = {
        ...
        /*
            Check for all task_status_id = COMPLETE_TASK_STATUS_ID for task under job.
            If all task is completed(tl_task_status_id = COMPLETE_TASK_STATUS_ID).
            The job is counted as completed
          */
        TlTasks: {
          every: {
