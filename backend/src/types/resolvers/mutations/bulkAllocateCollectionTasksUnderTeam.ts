...
*/
export const bulkAllocateCollectionTasksUnderTeam = mutationField('bulkAllocateCollectionTasksUnderTeam', {
  type: 'Boolean',
  args: {
    items: nonNull(list('bulkAllocateTasksUnderTeamInput'))
  },
  resolve: async (_parent, { items }, ctx) => {
    const collectionServiceTaskTypeId = 1;
    async function updateTasks(tl_job_id: number, tl_user_id: number) {
      ...
      await ctx.prisma.tlTasks.updateMany({
        where: {
          tl_job_id,
          tl_service_task_type_id: collectionServiceTaskTypeId
        },
        data: {
          tl_user_id,
          tl_team_id,
